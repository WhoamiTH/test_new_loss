...
for epoch in range(num_epochs):
    batch_x_pos, batch_y_neg = generate_transformed_batch_data(sample_method, positive_data, negative_data, batch_size)
    train_pre_x, train_pos_x, train_y = transform_data_to_train_form(batch_x_pos, batch_y_neg)

    input_pre_data = torch.Tensor(torch.from_numpy(train_pre_x).float())
    input_pos_data = torch.Tensor(torch.from_numpy(train_pos_x).float())
    input_label = torch.Tensor(torch.from_numpy(train_y).float())

    input_pre_data = input_pre_data.to(device)
    input_pos_data = input_pos_data.to(device)
    input_label = input_label.to(device)
    
    predict_pre = net(input_pre_data)
    predict_pos = net(input_pos_data) 
